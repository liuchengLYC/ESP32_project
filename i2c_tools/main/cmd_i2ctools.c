...
static int do_i2cwr_cmd(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&i2cwr_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, i2cwr_args.end, argv[0]);
        return 0;
    }

    int chip_addr = i2cwr_args.chip_address->ival[0];
    int write_len = i2cwr_args.write_data->count;
    int read_len = i2cwr_args.read_length->ival[0];

    if (write_len == 0 || read_len <= 0) {
        ESP_LOGE(TAG, "Invalid arguments: need write data and read length");
        return 1;
    }

    uint8_t *write_buf = malloc(write_len);
    uint8_t *read_buf = malloc(read_len);
    for (int i = 0; i < write_len; i++) {
        write_buf[i] = i2cwr_args.write_data->ival[i];
    }
    ...
    if (ret == ESP_OK) {
        ...
        ESP_LOGW(TAG, "Write-Read Failed");
    }
...
static void register_i2cwr(void)
{
    i2cwr_args.chip_address = arg_int1("c", "chip", "<chip_addr>", "Specify I2C device address");
    i2cwr_args.write_data = arg_intn("w", "write", "<write_bytes>", 1, 256, "Bytes to write before reading");
    i2cwr_args.read_length = arg_int1("r", "read", "<read_length>", "Number of bytes to read after write");
    i2cwr_args.end = arg_end(2);

    const esp_console_cmd_t i2cwr_cmd = {
        .command = "i2cwr",
        .help = "Write some bytes then read from the I2C device",
        .hint = NULL,
        .func = &do_i2cwr_cmd,
        .argtable = &i2cwr_args
    };
    ESP_ERROR_CHECK(esp_console_cmd_register(&i2cwr_cmd));
}
